...
| 指标 | 优化前 | 优化后 | 改善幅度 |
|------|--------|--------|----------|
| **平均心跳 RTT** | `0` ms | `~1.7` ms | ✅ 从无到有 |
| **断开次数** | 2次/40分钟 | 0次/22分钟 | ✅ 100% 减少 |
| **活跃连接** | 0 | 1 | ✅ 稳定 |
| **最长连接时长** | ~17分钟 | >22分钟+ | ✅ 持续稳定 |
| **重连机制** | 无 | 指数退避 | ✅ 新增 |

> **注**: `avgHeartbeatInterval` 在日志中指的是心跳往返时间（RTT），不是心跳发送间隔。心跳发送间隔约为 30 秒（由 MimoEngine 配置）。
...
平均心跳 RTT: 1.5-2 ms（优秀，网络通信正常）
心跳发送间隔: ~30 秒（MimoEngine 配置）
断开次数: 0 次
状态: ✅ 完全稳定
```
...
> **术语说明**:
> - **RTT (Round-Trip Time)**: 心跳请求-响应的往返时间，反映网络延迟（1-2ms 为优秀）
> - **心跳间隔**: 客户端发送心跳请求的频率（约 30 秒，由 MimoEngine 配置）
