 ...
 *
 * 独立的“页面内容 XPath 解析/扫描器”：
 * - 输入：tabId + 扫描参数
 * - 输出：候选元素的稳定 XPath 列表（主 frame + iframe，含 OOPIF）
 *
 * 说明：本类不处理 chrome.runtime.onMessage 等消息入口，避免后续功能重构时误改/误删核心扫描逻辑。
 */

import type { StagehandXPathItem, StagehandXPathScanResponse } from "../../types/stagehand-xpath"
import {
...
export type StagehandXPathScannerOptions = {
  maxItems: number
  selector: string
...
export class StagehandXPathScanner {
  /**
   * 防抖/去重：同一个 tabId 的扫描不并发执行，避免重复 attach debugger/重复构建 DOM 索引。
   */
  private readonly inFlightByTabId: Map<number, Promise<StagehandXPathScanResponse>> = new Map()

  public async scan(tabId: number, options: StagehandXPathScannerOptions): Promise<StagehandXPathScanResponse> {
    const existing = this.inFlightByTabId.get(tabId)
    if (existing) return await existing

    const promise = this.scanImpl(tabId, options).finally(() => {
  ...
  private async scanImpl(tabId: number, options: StagehandXPathScannerOptions): Promise<StagehandXPathScanResponse> {
    ...
    try {
      ...
      const getIndex = async (sid: string | undefined): Promise<SessionDomIndex> => {
        ...
        const idx = await buildSessionDomIndex(async (method, params) => await this.sendCdp(tabId, sid, method, params), pierce)
      ...
      for (const fid of orderedFrames) {
        ...
        for (const { backendNodeId, node } of pairs) {
          ...
          const xpath = fid === rootFrameId ? nodeAbs : mergeFrameXPath(absPrefix, relativizeXPath(bodyAbs, nodeAbs))

          const attrs = Array.isArray((node as any).attributes) ? ((node as any).attributes as string[]) : []
          ...
          const tagName = String((node as any).nodeName || "").toLowerCase()
          const id = attr("id") || undefined
          const className = attr("class") || undefined
          const textSnippet = attr("aria-label") || attr("title") || attr("alt") || attr("placeholder") || undefined
